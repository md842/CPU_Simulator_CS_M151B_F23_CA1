...
int main(int argc, char* argv[]){
	...
	*/
    ...
    int ALUMux = 0;
    // 0: Store ALU result to register
    // 1: Use ALU result as data memory address

    if (controlSignals[5]){ // RegWrite = 1
      if (controlSignals[2]){ // MemRe = 1
        // lw instruction
        ALUMux = 1;
        operand1 = myCPU.read_rs1(&instr);
      ...
      if (controlSignals[1]){ // MemWr = 1
        ...
        // blt instruction
        cout << "blt instruction: To be implemented!" << endl;
      }
    }

    // ALU
    // cout << "Operand 1: " << operand1 << "; Operand 2: " << operand2 << endl;
    int ALUResult = myCPU.ALUOperation(operand1, operand2);
    int toStore = ALUResult;
    if (ALUMux){ // If ALU result is a data memory address
    ...
    else // RegWr = 0; r-type, i-type, or lw
